...
def send_command(command):
    try:
        # Start the subprocess only if it's not running already - So the GUI open and works smoothly at first
        global recording_process
        if recording_process is None:
            print("[*] Starting manage_camera.py subprocess...")
            recording_process = subprocess.Popen(
            ...
            # Wait for the subprocess to set up the pipe (you can adjust the time if needed)
            time.sleep(1)  # Small delay to ensure the pipe is ready
